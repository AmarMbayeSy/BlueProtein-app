...
<section class="actualite-page">
    <div class="news-container">
        @php
            $counter = 1;
        @endphp
        @foreach($blocs->take(5) as $bloc)
            @if($counter == 1 || $counter == 4)
                <div class="news-card news-long-card">
            @elseif($counter == 2 || $counter == 3)
                <div class="news-card news-short-card">
            @elseif($counter == 5)
                <div class="news-card large-news-cards">
            @endif
                <img src="{{ asset('storage/' . $bloc->image_bloc) }}" alt="{{ $bloc->image_bloc }}">
                <div class="news-card-content">
                    <h3>{{ $bloc->titre_bloc }}</h3>
                    <p>{{ $bloc->contenu_bloc }}</p>
                </div>
            </div>
            @php
                $counter++;
